...
public class EffectHandler
{
    static public void Cast(Spell i_spell, GameObject i_spellCaster, BattleWorld i_world, Vector3 i_startPoint, Vector3 i_startDirection)
    {
        ...
        {
            ExecuteEffect(effect, i_spellCaster, i_world, i_startPoint, i_startDirection, i_spellCaster);
        }
    }

    static public void ExecuteEffect(Effect i_effect, GameObject i_spellCaster, BattleWorld i_world, Vector3 i_startPoint, Vector3 i_startDirection, GameObject i_hitReceiver)
    {
        Effect.Params p = new Effect.Params();
        p.caster = i_spellCaster;
        p.receiver = i_hitReceiver;
        p.world = i_world;
